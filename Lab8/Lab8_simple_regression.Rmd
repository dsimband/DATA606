---
title: "DATA606 - Introduction to linear regression"
...
- **rows - observations 1,458**
- **columns - variables 123**
...
- **form: linear**
- **direction: positive**
- **strength: moderate the points seem to be somewhat scattered**
...
- **y intercept: -4.035 - the regression line crosses the y axis at -4.035 or at pf_score of 0 there is a -4.035 expression control**
- **slope: 1.291 - for every point increase in pf_score the expression score will increase by 1.291**
...
y <- hfi %>% filter(between(pf_expression_control, 6.6 , 6.8)) %>% select(pf_expression_control, pf_score)
y

y %>% summary()

```
- **slightly underestimated - the mean score for a pf_expression_control value of 6.75 is 8.006**
...
- **no inherent pattern - the relationship is a strong linear relationship** 
...
- **yes the histogram is close to normal distribution and the normal probability plot is almost a straight line**
...
9.  Based on the residuals vs. fitted plot, does the constant variability condition 
    appear to be met?
- **yes - with the exception of the extremes. the values at the tail ends seems to be more distributed**
...
ggplot(hfi, aes(x=ef_legal_protection, y=pf_score)) + geom_point(alpha = 1/5) + stat_smooth(method = "lm", se = FALSE) + ggtitle("ef_legal_protection")
ggplot(hfi, aes(x=pf_association_sport, y=pf_score)) + geom_point(alpha = 1/5) + stat_smooth(method = "lm", se = FALSE) + ggtitle("pf_association_sport")
ggplot(hfi, aes(x=pf_rol_criminal, y=pf_score)) + geom_point(alpha = 1/5) + stat_smooth(method = "lm", se = FALSE) + ggtitle("pf_rol_criminal")
ggplot(hfi, aes(x=pf_ss_homicide, y=pf_score)) + geom_point(alpha = 1/5) + stat_smooth(method = "lm", se = FALSE)+  ggtitle("pf_ss_homicide")
```
...
- **similar positive linear relationship however there is more spread in the pf_score**
...
ggplot(hfi, aes(x=pf_rol_criminal, y=pf_score)) + geom_point(alpha = 1/5) + ggtitle("pf_rol_criminal")

m3 <- lm(pf_rol_criminal ~ pf_score, data = hfi)
summary(m3)
...
t1 <- hfi %>% select(pf_score ,pf_association_sport,pf_expression, ef_trade, ef_money, ef_legal_enforcement, ef_government_tax_income) %>% drop_na() 

t1.rcorr = rcorr(as.matrix(t1))
t1.rcorr
    
    
t1_cor = cor(t1, method = c("spearman"))
corrplot(t1_cor, title="Coorelation")
