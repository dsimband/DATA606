...
**yes - if the student uses the same seed to initialze the random function. no otherwise. The number should be fairly close**
...
8.  Since you have access to the population, simulate the sampling 
    ...
    ** I would assume the true proportions would be slighly over the 80% **
...
9.  Change your sample size from 15 to 150, then compute the sampling 
    ...
    ** I would guess the true proportions were 80% **
    
```{r}


sample_props150 <- global_monitor %>%
                    rep_sample_n(size = 15, reps = 150, replace = TRUE) %>%
                    count(scientist_work) %>%
                    mutate(p_hat = n /sum(n)) %>%
                    filter(scientist_work == "Benefits")
...
ggplot(data = sample_props150, aes(x = p_hat)) +
  ...
  labs(
    x = "p_hat (Benefits)",
    title = "Sampling distribution of p_hat",
    subtitle = "Sample size = 15, Number of samples = 150"
  )
...
10.  Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    ** the sampling distribution from 9 is smaller than the sampling distribution from 8**
