...
no_helmet2 <- no_helmet %>% drop_na(text_ind)


# no_helmet <- no_helmet %>%
#   mutate(text_ind = ifelse(is.na(text_while_driving_30d), "no" ,
#     ifelse(
#      text_while_driving_30d == "30", "yes", "no")
#     )
#   )
...
no_helmet2 %>% select(text_ind) %>% distinct()
...
-**H0 sleeping 10+ hours does not increase the likelihood of strength training every day**
...
sst_df %>%
  specify(response = sleep_ind, explanatory = strength_ind, success = "yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95)
