...
# plot histogram of calories from fat
...
**McDonald's**
The McDonald's distribution of calories from fat is right skewed and tends to be higher than distribution for Dairy Queen
+ Median 240
+ Mean 285

**Dairy Queen**
The Dairy Queen distribution of calories from fat is right skewed as well but to a lesser extent
+ Max 620 - about half the max calories found at McDonald's
+ Median 220 - when compared to the median at McDonald's this is 20 points lower
+ Mean 260 - when compared to the mean at McDonald's this is 20 points lower
...
ggplot(data = mcdonalds, aes(x = cal_fat)) +
        geom_blank() +
        geom_histogram(aes(y = ..density..)) +
        stat_function(fun = dnorm, args = c(mean = mmean, sd = msd), col = "tomato") +
        labs(
          title = paste(
         "McDonald's")
        )
...
**The distribution of calories in the dairy queen data is closer to normal or bell shaped than the distribution of calories for McDonald's.The distribution of the McDonald's data appears to skew right.** 
...
**It looks similar with some deviation specifically around the mean and the endpoints**
...
**The cal from fat distributions in the McDonald's data clearly indicate that the data is not normally distributed**
    
```{r}
ggplot(data = mcdonalds, aes(sample = cal_fat)) + 
  geom_line(stat = "qq")

sim_norm <- rnorm(n = nrow(mcdonalds), mean = mmean, sd = msd)
...
**what is the probability that an item on the Arbys menu has less than 700 calories?**
...
pnorm(q = 700, mean = mean(arbys$calories), sd = sd(arbys$calories))
...
**what is the probability that an item on the Arby's menue has less than 20 grams of fat**
30.9%
...
pnorm(q = 20, mean = mean(arbys$total_fat), sd = sd(arbys$total_fat))
...
**Burger King and Arby's are the closest to normal distributions **
...
**when you see stepwise paterns it usually corresponds to gaps in records that correspond to those values. You can see from the histogram above that the records between 2000 and 3000 is sparse **
...
**Based on the plots below I would say that the sodium levels for selected items at Sonic skew right. That is confirmed by the histogram for sodium levels using bin number of 25**
