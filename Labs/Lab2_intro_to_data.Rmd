...
**- The 2nd histogram with the bin width of 15 is the most detailed. The default bin width of 30 is used for the first histogram. And the final histogram uses a bin width of 150 that obscures most of the variation in departure time. 
- Using a bin width of 15 you can see that some of the flights leave ahead of schedule.Roughly the same number of flight leave between 1 and 15min ahead of schedule when compared to the number of flights of leave 1min to 15min late. This detail is obscured when using the default bin width of 30 and the bin width of 150.**
...
**- DL and UA have the largest interquartile range (22)**
    
    
```{r answers questions 1a}
sfo_feb_flights %>%
  group_by(carrier) %>%
...
dl_aa_ua <- nycflights %>%
  filter((carrier == "AA" | carrier == "DL" | carrier == "UA") , dep_delay <=-5 )
...
** if your flight leaves 12 min before scheduled departure you can expect to reach your destintion on time. dep_delay for 100% arrivial on time is -12**
...
This document is available at [[**RPubs**]](https://rpubs.com/dsimbandumwe/808292)
and on [[**Github**]](https://github.com/dsimband/DATA606/blob/1b16c2d9672058cec0f416b968d8a66c2bc0d80a/Labs/Lab2_intro_to_data.Rmd)
